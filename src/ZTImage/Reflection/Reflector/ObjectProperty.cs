...
namespace ZTImage.Reflection.Reflector
{
    ...
    public sealed class ObjectProperty
    {
        ...
        private void Init()
        {
            ...
            object[] orders=_Member.GetCustomAttributes(typeof(PropertyOrderAttribute), false);
            if (orders != null && orders.Length > 0)
            {
                Order = (orders[0] as PropertyOrderAttribute).Order;
            }
            else
            {
                Order = -1;
        ...
        /// </summary>
        public ZTGetter Getter { get; private set; }
        ...
        {
            var attr=this._Member.GetAttribute<UnSerializedAttribute>(true);
            if (attr == null)
            {
                return true;
            }
        ...
        {
            return this._Member.GetAttribute<T>(inherit);
        }
        ...
        /// <param name="inherit"></param>
        /// <returns></returns>
        public List<T> GetAttributes<T>(bool inherit) where T : Attribute
        {
            return this._Member.GetAttributes<T>(inherit);
        }
